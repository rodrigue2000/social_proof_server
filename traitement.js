 // traitement.js - Version modifiée
...
// --- Commande multitâches (NOUVEAU) ---
async function traiterCommandeApprouvee(customMetadata) {
  const { commandeId, demandeurId, montant } = customMetadata;
  
  if (!commandeId) {
    console.error('traiterCommandeApprouvee : commandeId manquant dans custom_metadata');
    return;
  }
  ...
  try {
    ...
    // 2. Extraire les informations de la commande
    ...
    // 3. Créer les N tâches dans une transaction
    ...
    for (let i = 0; i < nombreTaches; i++) {
      ...
      batch.set(demandeDoc, {
        ...
        // Infos supplémentaires pour le suivi
        montantUnitaire: services[0]?.prixUnitaire || 0,
      });
    }
    ...
    console.error(`❌ Erreur lors du dispatching de la commande ${commandeId}:`, error);
...
module.exports = { 
  ...
  // Export des fonctions individuelles pour les tests éventuels
